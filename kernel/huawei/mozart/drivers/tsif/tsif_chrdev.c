...
static int tsif_open(struct inode *inode, struct file *file)
{
	int rc;
...
static int tsif_release(struct inode *inode, struct file *filp)
{
			 ...
			 loff_t *f_pos)
{
	...
	if (copy_to_user(buf, the_dev->data_buffer + the_dev->rptr, avail)) {
		dev_err(the_dev->dev,"copy to user fail\n");
		return -EFAULT;
	}
...
static unsigned int tsif_poll(struct file *filp, struct poll_table_struct *table)
{
	struct tsif_chrdev *the_dev = filp->private_data;
	tsif_get_state(the_dev->cookie, &the_dev->ri, &the_dev->wi,
		       &the_dev->state);
	poll_wait(filp, &the_dev->wq_read, table);
	if (the_dev->ri != the_dev->wi){
...
static dev_t tsif_dev1; /**< next not yet assigned dev_t */
...
static int tsif_init_one(struct tsif_chrdev *the_dev, int index)
{
	...
	rc = cdev_add(&the_dev->cdev, tsif_dev1, 1);
...
static int __init mod_init(void)
{
	...
	if (rc)
		goto err_init1;
	printk(KERN_INFO"tsif chrdev is ok!\n");
	return 0;
err_init1:
