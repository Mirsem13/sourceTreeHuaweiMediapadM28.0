...
//#include <linux/log_jank.h> //###Temporarily removed###
...
static int capacity_changed(struct bq_bci_device_info *di)
{
    ...
    */
    ...
    if (curr_capacity < 2 )
    {
        ...
        hwlog_err("error capacity reported, capacity = %d\n", curr_capacity);
        return 0;
    }
...
int check_batt_bad_curr_sensor(char* buf)
{
    ...
    if ((!di->usb_online) && (!di->ac_online) && (0 == plugged) && (current_now > CURRENT_OFFSET))
    {
        times++;
    }
    else
...
int check_discharge_curr_overhigh(char* buf)
{
    ...
    {
        current_now[i] = -hisi_battery_current();
        if ((!di->usb_online) && (!di->ac_online) && current_now[i] < (-7000))
        {
            msleep(100);
        }
        else
...
static void bq_get_error_info(struct bq_bci_device_info *di)
{
    int i;
    char buf[1024];
        ...
        if (batt_dsm_array[i].notify_enable)
        {
            if (batt_dsm_array[i].check_error(buf))
            {
                if (!dsm_client_ocuppy(battery_dclient))
                {
                    ...
                    dsm_client_notify(battery_dclient, batt_dsm_array[i].error_no);
...
static ssize_t bq_bci_poll_charge_start_event(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t bq_bci_set_charge_event(struct device *dev,
...
static DEVICE_ATTR(poll_charge_start_event, (S_IWUSR | S_IRUGO),
                bq_bci_poll_charge_start_event,
                bq_bci_set_charge_event);
