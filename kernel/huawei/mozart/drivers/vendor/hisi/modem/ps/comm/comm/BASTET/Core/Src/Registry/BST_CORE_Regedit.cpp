    ...
    BST_VOID                 *const pData)
{
                        ...
                        lstNext( ( NODE * )pstPidNode ) )
    {
        ...
        if ( BST_NULL_PTR != pstPidNode->pcCallBack )
        {
            usInquireLen            = pstPidNode->pcCallBack->Inquired
    ...
    BST_VOID                   *pData )
{
      ...
      || ( BST_CORE_PID_VAR_LEN == pstParamAttri->enLenType ) )
    {
        ...
        if ( BST_NULL_PTR == pstPid->pcCallBack )
        {
            BST_RLS_LOG( "BST_CORE_CRegedit::Save pstPid->pcCallBack=NULL" );
            return BST_ERR_INVALID_PTR;
        }
        enRet                   = pstPid->pcCallBack->Configed ( pstPid->stParamInfo.enPid,
                                                                 usDataLen,
                                                                ( void * )pData,
                                                                &pNewData );
    ...
    const void               *const pValue )
{
    ...
    */
    ...
    pstNewRegItem->pcCallBack               = pCallBack;
