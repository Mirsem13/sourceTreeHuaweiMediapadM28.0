    ...
    const BST_APP_EXCGREQ_TYPE_ENUM_UINT8 enCmdType )
{
    ...
    usLen               = CreateRqstBody( pstWbxmlBody, enCmdType );
    ...
    BST_LIB_StrAddBytes( m_pstrRequest, m_pstrWbxml->pData, usLen );
    ...
    BST_APP_EXCGREQ_TYPE_ENUM_UINT8 enCmdType )
{
    ...
    BST_LIB_StrAddStr( m_pstrRequest, "Authorization: Basic " );
    /*“账号：密码” base64编码*/
    BST_LIB_StrAddStr( m_pstrRequest, strUserNameAndPass  );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    BST_LIB_StrAddStr( m_pstrRequest, "MS-ASProtocolVersion: " );
    BST_LIB_StrAddStr( m_pstrRequest, pstHttpReqHead->pstPrtcVer->pData );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    BST_LIB_StrAddStr( m_pstrRequest, "User-Agent: " );
    BST_LIB_StrAddStr( m_pstrRequest, pstHttpReqHead->pstUsrAgent->pData );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    BST_LIB_StrAddStr( m_pstrRequest, "Accept-Encoding: " );
    BST_LIB_StrAddStr( m_pstrRequest, pstHttpReqHead->pstEncoding->pData );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    BST_LIB_StrAddStr( m_pstrRequest, "Content-Type: application/vnd.ms-sync.wbxml" );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    if ( SYNC_CMD == enCmdType )
    {
        BST_LIB_StrAddStr( m_pstrRequest, "X-MS-PolicyKey: " );
        BST_LIB_StrAddStr( m_pstrRequest, pstHttpReqHead->pstPolicyKey->pData );
        BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    }
    BST_LIB_StrAddStr( m_pstrRequest, "Content-Length: " );
    BST_LIB_StrAddStr( m_pstrRequest, strBodyLen );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    BST_LIB_StrAddStr( m_pstrRequest, "Host: " );
    BST_LIB_StrAddStr( m_pstrRequest, pstHttpReqHead->pstHostName->pData );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    BST_LIB_StrAddStr( m_pstrRequest, "Connection: Keep-Alive" );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
    BST_LIB_StrAddStr( m_pstrRequest, BST_EMAIL_HTTP_CRLF );
}
BST_UINT16 BST_APP_CExcgHttp::CreateRqstBody(
    const BST_EMAIL_WBXML_BODY_STRU    *pstWbxmlBody,
    BST_APP_EXCGREQ_TYPE_ENUM_UINT8     enCmdType )
{
    BST_UINT16          usLen;
    ...
    switch ( enCmdType )
    {
        ...
        case SYNC_CMD:
            usLen      += CreateSyncWbxml( pstWbxmlBody );
            break;
    ...
    const BST_EMAIL_WBXML_BODY_STRU *pstWbxmlBody )
{
    ...
    CreateWbxmlTag( BST_EMAIL_PING_PING, BST_FALSE );
    /*心跳间隔*/
    CreateWbxmlData( "480", BST_EMAIL_PING_HEARTBEAT_INTERVAL );
    CreateWbxmlTag( BST_EMAIL_PING_FOLDERS, BST_FALSE );
    CreateWbxmlTag( BST_EMAIL_PING_FOLDER, BST_FALSE );
    /*需要AP传*/
    CreateWbxmlData( pstWbxmlBody->pstClctId->pData, BST_EMAIL_PING_ID );
    /*同步邮件*/
    CreateWbxmlData( "Email", BST_EMAIL_PING_CLASS );
    ...
    const BST_EMAIL_WBXML_BODY_STRU *pstWbxmlBody )
{
    ...
    CreateWbxmlTag( BST_EMAIL_SYNC_SYNC, BST_FALSE );
    CreateWbxmlTag( BST_EMAIL_SYNC_COLLECTIONS, BST_FALSE );
    CreateWbxmlTag( BST_EMAIL_SYNC_COLLECTION, BST_FALSE );
    CreateWbxmlData( pstWbxmlBody->pstSyncKey->pData, BST_EMAIL_SYNC_SYNC_KEY );
    /*要同步的文件夹ID号*/
    CreateWbxmlData( pstWbxmlBody->pstClctId->pData, BST_EMAIL_SYNC_COLLECTION_ID );
    /*所有被删除的项目都移动到删除文件夹中*/
    CreateWbxmlData( "1", BST_EMAIL_SYNC_DELETES_AS_MOVES );
    /*让server返回发生变化*/
    CreateWbxmlTag( BST_EMAIL_SYNC_GET_CHANGES, BST_TRUE );
    /*server返回变化项目的最大值*/
    CreateWbxmlData( pstWbxmlBody->psWinSize->pData, BST_EMAIL_SYNC_WINDOW_SIZE );
    CreateWbxmlTag( BST_EMAIL_SYNC_OPTIONS, BST_FALSE );
    /*向前一个月内的变化*/
    CreateWbxmlData( "4", BST_EMAIL_SYNC_FILTER_TYPE );
    BST_LIB_StrAddByte( m_pstrWbxml, BST_EMAIL_SWITCH_PAGE );
    BST_LIB_StrAddByte( m_pstrWbxml, BST_EMAIL_BASE );
    CreateWbxmlTag( BST_EMAIL_BASE_BODY_PREFERENCE, BST_FALSE );
    CreateWbxmlData( "2", BST_EMAIL_BASE_TYPE );
    /*邮件截断的大小，可以不用修改*/
    CreateWbxmlData( "200000", BST_EMAIL_BASE_TRUNCATION_SIZE );
    BST_LIB_StrAddByte( m_pstrWbxml, BST_EMAIL_END );
    BST_LIB_StrAddByte( m_pstrWbxml, BST_EMAIL_END );
    BST_LIB_StrAddByte( m_pstrWbxml, BST_EMAIL_END );
    BST_LIB_StrAddByte( m_pstrWbxml, BST_EMAIL_END );
...
BST_LIB_StrStru *BST_APP_CExcgHttp::GetRequest( BST_VOID )
{
