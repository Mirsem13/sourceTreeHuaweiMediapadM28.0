...
static int wl_android_get_rssi(struct net_device *net, char *command, int total_len)
{
	...
	if ((total_len - bytes_written) < (strlen(" rssi -XXX") + 1))
		return -ENOMEM;
	bytes_written += scnprintf(&command[bytes_written], total_len, " rssi %d", rssi);
...
#ifdef DHD_REOPEN_HW
	else if (strnicmp(command, CMD_RECONNECT, strlen(CMD_RECONNECT)) == 0) {
		DHD_INFO(("%s, Receive Reconnect Command\n", __FUNCTION__));
		ret = wl_cfg80211_hang(net, WLAN_REASON_RECOVERY);
	}
#endif
