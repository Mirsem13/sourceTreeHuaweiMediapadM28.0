	...
	for(i = 0; i < 4; i++){
		...
		if((param_type == TEEC_MEMREF_TEMP_INPUT) ||
		   ...
		   (param_type  == TEEC_MEMREF_TEMP_INOUT)){
			...
			if(/*ZERO_OR_NULL_PTR(temp_buf)*/!temp_buf){
	...
	if(tee_ret != 0){
		while(tee_ret == TEEC_PENDING2 || tee_ret == TEEC_PENDING){
			if(tee_ret == TEEC_PENDING2){
				...
				tee_ret = TC_NS_incomplete_proceed(smc_cmd, agent_id, flags);
			}
...
error1:
	if (tee_ret == TEEC_ERROR_SHORT_BUFFER) {
		TCDEBUG("Short buffer operation!\n");
		/* update size */
		if(operation){
			ret = update_client_operation(dev_file, client_context,
						      operation,
