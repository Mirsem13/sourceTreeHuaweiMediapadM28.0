    ...
    BST_DSPP_PKTPC_T         usPicNum )
{
    ...
    if ( usPicNum > pstRcvInfo->stRcvMng.usPicAmount )
    {
        return BST_ERR_UNREACHABLE;
    }
    ...
    BST_UINT16               usSduLen )
{
     ...
     */
    if ( 1 == stPktPicMsg->usPicAmount )
    {
        pstDsppHead->usLength   = usSduLen - BST_DSPP_TPA_HEAD_LEN;
        pstDsppHead->ulZeroCopy = BST_TRUE;
    ...
    pstDsppHead->ulZeroCopy     = BST_FALSE;
    ...
    pstDsppHead->pucData        = ( BST_UINT8 * )BST_OS_MALLOC ( pstDsppHead->usLength );
    ...
    BST_UINT16               usSduLen )
{
    ...
    switch ( stPktPicMsg->enPrimitive )
    {
        ...
        case BST_DSPP_PRMTV_ACK:
        {
            ...
            pTrsUlVerA->RxAck ( stPktPicMsg->usPktNum, stPktPicMsg->usPicNum );
    ...
    BST_DSPP_TP_PKTMSG_STRU *pstPacketInfo )
{
    ...
    pstPktRcvBuf->stRcvMng.usPicNum     = BST_DSPP_TP_FIRST_PIECE_NUMBER;
    pstPktRcvBuf->stRcvMng.usAmountLen  = 0;
    lstInit( &pstPktRcvBuf->stPicList );
    pstPktRcvBuf->stRcvMng.enCpVer      = BST_DSPP_VER_INVALID;
    pstPktRcvBuf->ulTimerId             = BST_OS_TimerCreateCpp ( this, BST_NULL_PTR );
    return pstPktRcvBuf;
}
    ...
    BST_DSPP_PKTSN_T usInPktNumber )
{
    ...
    BST_RLS_LOG( "BST_DSPP_CTrsDlVa::FindRcvInfo Err" );
    return BST_NULL_PTR;
}
    ...
    BST_UINT16               usLength )
{
    ...
    pstPacketInfo->usPicAmount  = BST_DSPP_U8_TO_U16
                                ( pucData[BST_DSPP_TP_PIC_AMT_L],
                                  pucData[BST_DSPP_TP_PIC_AMT_H] );
    pstPacketInfo->usPicNum     = BST_DSPP_U8_TO_U16
                                ( pucData[BST_DSPP_TP_PIC_NUM_L],
                                  pucData[BST_DSPP_TP_PIC_NUM_H] );
    ...
    BST_UINT16               usInLen )
{
    ...
    ucRrnVal                    = CheckPicNumber ( pstRcvItem, pstPacketInfo->usPicNum );
    if ( BST_NO_ERROR_MSG != ucRrnVal)
    {
    ...
    if ( BST_DSPP_TP_FIRST_PIECE_NUMBER == pstPacketInfo->usPicNum )
    {
    ...
    pstPieceInfo->usPicNum      = pstPacketInfo->usPicNum;
    ...
    if ( pstRcvItem->stRcvMng.usPicAmount
         == lstCount( &pstRcvItem->stPicList ) )
    {
        BST_OS_TimerStop ( pstRcvItem->ulTimerId );
        return pstRcvItem;
    }
    else if( pstRcvItem->stRcvMng.usPicAmount
             < lstCount( &pstRcvItem->stPicList ) )
    {
        ClrRcvManager ( pstPacketInfo->usPktNum );
    }
    ...
    BST_UINT16               usSduLen )
{
    ...
    pucBuf[ BST_DSPP_TP_PKT_NUM_H ]     = BST_DSPP_U16_TO_U8H
                                        ( pstPacketInfo->usPktNum );
    pucBuf[ BST_DSPP_TP_PIC_AMT_L ]     = BST_DSPP_U16_TO_U8L
                                        ( pstPacketInfo->usPicAmount );
    pucBuf[ BST_DSPP_TP_PIC_AMT_H ]     = BST_DSPP_U16_TO_U8H
                                        ( pstPacketInfo->usPicAmount );
    pucBuf[ BST_DSPP_TP_PIC_NUM_L ]     = BST_DSPP_U16_TO_U8L
                                        ( pstPacketInfo->usPicNum );
    pucBuf[ BST_DSPP_TP_PIC_NUM_H ]     = BST_DSPP_U16_TO_U8H
                                        ( pstPacketInfo->usPicNum );
    ...
    BST_DSPP_HEAD_STRU *const   pstTxHeadInfo )
{
    ...
    stPktPicMsg.usPicNum        = BST_DSPP_TP_FIRST_PIECE_NUMBER;
    stPktPicMsg.enPrimitive     = pstTxHeadInfo->enPrmvType;
    stPktPicMsg.enCpVer         = pstTxHeadInfo->enCpVer;
    stPktPicMsg.usPicAmount     = usSduLen / BST_DSPP_TPA_MAX_SDU;
    if ( 0 != ( usSduLen % BST_DSPP_TPA_MAX_SDU ) )
    {
        stPktPicMsg.usPicAmount+= 1;
    }
...
BST_ERR_ENUM_UINT8 BST_DSPP_CTrsUlVa::TxAck ( BST_DSPP_TP_PKTMSG_STRU *pstAckPktMsg )
{
    ...
    stPacketMsg.usPicAmount     = pstAckPktMsg->usPicAmount;
    stPacketMsg.usPicNum        = pstAckPktMsg->usPicNum;
