...
BST_VOID BST_SRV_CAsCtrl::ProcRrcEvent( BST_UINT32 ulLength, BST_VOID *pvData )
{
    ...
    /*
     * 如果当前进入连接态，则立即停止拆链定时器，并通知AS底层RRC状态发生变化
     */    
    ...
    pAsDriver->pfIoCtrl( BST_DRV_CMD_SET_TRX_ENABLE, &ulTrxRptFlag );
...
BST_VOID BST_SRV_CAsCtrl::ProcDataTrs( BST_VOID )
{
    ...
    BST_RLS_LOG ( "BST_SRV_CAsCtrl::ProcDataTrs : BST_AS_EVT_L3_AP_TRS_RCV_IND EnableFlag is FALSE.");
    ...
    BST_VOID               *pvData )
{
    switch( enEvent )
    {
        ...
        case BST_AS_EVT_L3_AP_TRS_RCV_IND:
            ProcDataTrs();
            break;
...
BST_SRV_CAsCtrl::BST_SRV_CAsCtrl( BST_VOID )
    : m_ulFdTimerId( BST_OS_INVALID_TIMER_ID ), m_ulReportTimerId( BST_OS_INVALID_TIMER_ID )
{
    m_VoteRrcEnableFlag                 = BST_TRUE;
    m_ulFdTimerId                       = BST_OS_TimerCreateCpp( this, BST_NULL_PTR );
    m_ulReportTimerId                   = BST_OS_TimerCreateCpp( this, BST_NULL_PTR );
    RegAsNotice( BST_AS_EVT_L3_RRC_STATE );
    RegAsNotice( BST_AS_EVT_L3_AP_TRS_RCV_IND );
